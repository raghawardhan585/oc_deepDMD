...
def transfer_current_ocDeepDMD_run_files():
    ...
    destination_folder = '/Users/shara/Box/YeungLabUCSBShare/Shara/DoE_Pputida_RNASeq_DataProcessing/System_' + str(SYSTEM_NUMBER) + '/Direct_nn'
...
def get_all_run_info(SYSTEM_NO,RUN_NO,sess):
    ...
    run_folder_name = sys_folder_name + '/Direct_nn/RUN_' + str(RUN_NO)
...
def generate_predictions_pickle_file(SYSTEM_NO, ls_process_runs):
    ...
    if os.path.exists(sys_folder_name + '/dict_predictions_Direct_nn.pickle'):
    ...
    ls_all_run_indices = []
    for folder in os.listdir(sys_folder_name+'/Direct_nn'):
    ...
    for run in ls_unprocessed_runs:
        ...
        for data_index in dict_indexed_data.keys():
            ...
            # 1 - step predictions
            x_next = dict_params['f'].eval(feed_dict={dict_params['xp_feed']: X_scaled})
            x_1step = np.concatenate([copy.deepcopy(X_scaled[0:1, :]),x_next[0:-1]],axis=0)
            # N - step predictions
            x_nstep = copy.deepcopy(X_scaled[0:1,:])
            for i in range(1,X_scaled.shape[0]):
                x_nstep = np.concatenate([x_nstep, dict_params['f'].eval(feed_dict={dict_params['xp_feed']: x_nstep[-1:]})])
            dict_predictions[run][data_index] = {'X_scaled':X_scaled, 'X_one_step_scaled': x_1step,'X_n_step_scaled': x_nstep, 'X': dict_indexed_data[data_index]['X']}
            dict_predictions[run][data_index]['X_one_step'] = oc.inverse_transform_X(x_1step, SYSTEM_NO)
            dict_predictions[run][data_index]['X_n_step'] = oc.inverse_transform_X(x_nstep, SYSTEM_NO)
            dict_predictions[run][data_index]['Y_scaled'] = dict_DATA_i['Y']
            dict_predictions[run][data_index]['Y'] = dict_indexed_data[data_index]['Y']
            dict_predictions[run][data_index]['Y_one_step_scaled'] = dict_params['g'].eval(feed_dict={dict_params['xp_feed']: x_1step})
            dict_predictions[run][data_index]['Y_n_step_scaled'] = dict_params['g'].eval(feed_dict={dict_params['xp_feed']: x_nstep})
            dict_predictions[run][data_index]['Y_one_step'] = oc.inverse_transform_Y(dict_predictions[run][data_index]['Y_one_step_scaled'], SYSTEM_NO)
            dict_predictions[run][data_index]['Y_n_step'] = oc.inverse_transform_Y(dict_predictions[run][data_index]['Y_n_step_scaled'], SYSTEM_NO)
...
def get_error(ls_indices,dict_XY):
    ...
    for i in ls_indices:
        ...
        all_errors = np.concatenate([np.square(dict_XY[i]['X_scaled'] - dict_XY[i]['X_n_step_scaled']),
                                     np.square(dict_XY[i]['Y_scaled'] - dict_XY[i]['Y_n_step_scaled'])], axis=1)
