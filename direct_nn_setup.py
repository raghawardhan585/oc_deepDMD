...
for items in ls_files:
    ...
    items.write('# Gen syntax: [interpreter] [code.py] [device] [sys_no] [run_no] [n_layers] [n_nodes] [write_to_file] \n')
running_code = 'direct_nn_identification.py'
ls_run_no =[0,0,0]
...
for run_no in ls_runs:
    print(run_no)
    dict_error[run_no] = {}
    dict_error[run_no]['train'] = dn.get_error(ls_train_curves, d[run_no])
    dict_error[run_no]['valid'] = dn.get_error(ls_valid_curves, d[run_no])
    dict_error[run_no]['test'] = dn.get_error(ls_test_curves, d[run_no])
