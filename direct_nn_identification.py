...
def objective_func_state(dict_feed,psix):
    ...
    SST = tf.math.reduce_sum(tf.math.square(y_true), axis=0)
...
hidden_vars_list[-1] = num_states + num_outputs  # The last hidden layer being declared as the output
...
with tf.device(DEVICE_NAME):
    ...
    dict_model_metrics = objective_func_state(dict_feed, psix)
...
tf.compat.v1.add_to_collection('f', dict_psi['f'])
tf.compat.v1.add_to_collection('g', dict_psi['g'])
tf.compat.v1.add_to_collection('xp_feed', xp_feed)
all_tf_var_names =['f','g','xp_feed']
