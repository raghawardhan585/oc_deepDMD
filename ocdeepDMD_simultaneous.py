...
ls_dict_training_params.append(dict_training_params)
dict_training_params = {'step_size_val': 0.1, 'train_error_threshold': float(1e-7), 'valid_error_threshold': float(1e-7), 'max_epochs': 40000, 'batch_size': 100 }
ls_dict_training_params.append(dict_training_params)
...
def objective_func(dict_feed,dict_psi,dict_K):
    ...
    dict_model_perf_metrics['loss_fn'] = tf.math.reduce_mean(tf.math.square(psiXf_prediction_error)) + tf.math.reduce_mean(tf.math.square(Yf_prediction_error))
...
x_hidden_vars_list[-1] = x_deep_dict_size + num_outputs + y_deep_dict_size # The last hidden layer being declared as the output
